// VAMETRIX Sales OS — Client Settings: everything the client edits themselves.
// Knowledge · Catalog · Integrations (WhatsApp / Meta ads token / lead forms).
// Server actions call the embedded engine's own functions — one code path.
    ...
    <div className="p-6 space-y-6 max-w-5xl">
      ...
      <div className={cardCls}>
        ...
        <form action={addKnowledge} className="space-y-2">
          ...
          <textarea name="content" rows={4} placeholder="Paste the knowledge text — FAQs, policies, product details…" className={inputCls} />
      ...
      <div className={cardCls}>
        ...
        <p className="text-xs text-zinc-500 mb-4">Floor price is the AI&apos;s hard negotiation limit — it can never quote below it.</p>
        <div className="space-y-2 mb-4">
          {(products ?? []).filter(p => p.active).map(p => (
            <form key={p.sku} action={deactivateProduct} className="flex items-center gap-3 text-sm">
              ...
              <span className="text-zinc-400">₹{Number(p.list_price).toLocaleString()} / floor ₹{Number(p.min_price).toLocaleString()} / MOQ {p.moq}</span>
              <button className="text-red-400 hover:text-red-300 text-xs">remove</button>
            </form>
          ))}
          {(products ?? []).filter(p => p.active).length === 0 && (
            <div className="text-sm text-amber-400/80">⚠ No active products — the AI cannot generate quotes until the catalog is added.</div>
          )}
        </div>
        <form action={saveProduct} className="grid grid-cols-2 md:grid-cols-6 gap-2">
          <input name="sku" placeholder="SKU" className={inputCls} />
          <input name="name" placeholder="Product name" className={inputCls} />
          <input name="list_price" placeholder="List ₹" className={inputCls} />
          <input name="min_price" placeholder="Floor ₹" className={inputCls} />
      ...
      <div className={cardCls}>
        ...
        <div className="space-y-2 text-sm">
          ...
          {(templates ?? []).length === 0 && <div className="text-zinc-500">None yet — the AI creates them when first needed.</div>}
        </div>
      </div>
    </div>
  );
